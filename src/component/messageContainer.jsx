...
let socket = io.connect(import.meta.env.VITE_APP_BACKEND_URL, { transports: ["polling", "websocket"], 'path': '/' })
...
export const MessageContainer = ({ selectedUser, setOnline, setSelectedUser, currentuser, setView, online, profile, setProfile }) => {
  ...
  return (
    <>
      {selectedUser ? <> { loading ? <div className='h-[100vh] flex flex-col justify-center items-center'>
        <ThreeCircles
          height="100"
          width="100"
          color="#7700cf"
          ...
          middleCircleColor=""
        />
      </div> : <div className='p-4 h-[97vh] md:h-[98vh] overflow-auto'>
        {/* message header */}
        <div className='flex'>
          <div className='w-[15%] md:w-[5%] flex items-center'><img src={selectedUser?.profilePicture?.name ? `${selectedUser?.profilePicture.imgSrc}` : "/logo1.jpg"} className='w-[2.3rem] h-[2.3rem] rounded-md' alt="avatar" /></div>
